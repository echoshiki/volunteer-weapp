...
export const BidRecordCard = ({ record, onClickDemand, onGoToOrder, onEdit, className }: BidRecordCardProps) => {
	const canEdit = record.status === 'pending';
	return (
		<View className={`flex flex-col ${className || ''}`}>
			{/* 头部：需求单名称与我的报价状态 */}
			<View className="flex justify-between items-center mb-2" onClick={() => onClickDemand?.(record.demandId)}>
			...
			{/* 详情与我当时提交的报价 */}
			<View className="flex flex-col gap-3 mb-3" onClick={() => onClickDemand?.(record.demandId)}>
				<View className="flex items-center gap-2">
					<Text className="text-xs text-text-muted">报价时间：{record.createTime.split(' ')[0]}</Text>
				</View>
			...
			{record.status === 'selected' && record.employerPhone && (
				<View className="bg-blue-50/50 border border-blue-100 rounded p-3 mb-3 flex justify-between items-center">
					<View className="flex flex-col gap-1">
						<Text className="text-xs text-blue-800 font-medium">雇主已选您！请立刻联系对方：</Text>
			...
			<View className="flex justify-between items-center">
				...
				<View className="flex items-center gap-2">
					...
					{/* 查看需求详情 */}
					<Button variant="ghost" size="sm" onClick={() => onClickDemand?.(record.demandId)}>
						查看原需求
					</Button>

					{/* 如果中标了，引导服务方去查看进行中的服务订单 */}
					{record.status === 'selected' && (
						<Button variant="primary" size="sm" onClick={() => onGoToOrder?.(record.orderId)}>
							查看服务订单
						</Button>
					)}
				</View>
