...
export default function ApplyVolunteerPage() {
	const { form, updateField, onUploadIdCard, handleSave, isSubmitting } = useVolunteerApply();
		...
		<Page className="pt-4 pb-10 bg-main-bg">
			<View className="container-x flex flex-col gap-4">
				...
				<Cell className="flex flex-col gap-4 py-8">
					...
					<View className="flex justify-between gap-4 mt-2 mb-1">
						{/* 身份证正面 (人像面) */}
						<UploadBox
							className="flex-1"
							value={form.idCardFront}
							onClick={() => onUploadIdCard('idCardFront')}
						...
						<UploadBox
							...
							onClick={() => onUploadIdCard('idCardBack')}
