...
export default function UserApplyPage() {
	...
	// 数据：认证申请记录集合
	const { data, isLoading } = useApplyHistory({});
	const historyRecords = useMemo(() => data?.pages.flatMap((page: any) => page.list || []) || [], [data]);

	const getTenantCertificationStatus = () => {
		// 在历史记录查询属于当前大区的记录
		const currentTenantRecords = historyRecords.filter((item) => Number(item.tenantId) === currentTenantId);
		...
		<Page className="container-x">
			...
			<View className="flex flex-col gap-4">
				...
				<EntryCard
					...
					theme="blue"
					disabled={getTenantCertificationStatus().isApproved}
